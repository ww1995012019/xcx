...
Page({
  ...
  gotopath: function (e) {
    console.log(e)
    var page = e.currentTarget.dataset.indexp
    wx.navigateTo({
      url: page,
    })
  ...
  addRequestCart: function (gid, number) {
    ...
    data.gid = gid;
    data.aid = wx.getStorageSync("index-aid");
    data.num = number;
    console.log(data);
  ...
  getToalCartsNum: function () {
    var data={};
    var that=this;
    
    data.map = 'applet_sequence_carts_list';
    data.id = wx.getStorageSync("hd_id");
    wx.request({
      url: app.globalData.requestUrl,
      data: data,
      success: function (res) {
        
        var goods_list = that.data.goodsList;
        ...
        if (res.data.ec == 200) {
          console.log(res.data)
            ...
            console.log("购物车")
                ...
                for (var h = 0; h < res.data.cartList.length;h++){
                  for (var j = 0; j < goods_list.length; j++) {
                  
                    if (res.data.cartList[h]['gid'] == goods_list[j]['id']) {
                          goods_list[j].num = res.data.cartList[h]['num']
                          goods_list[j].xiaomi= res.data.cartList[h]['num']
                        }
  ...
  submitOrder: function (e) {
    ...
    wx.setStorageSync("index-aid", asaId)
    if (double == 1) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.ec == 200) {
          ...
          // } else {
            ...
            console.log(res.data.goods_row.cartList)
          ...
          wx.setStorageSync("hd_id",allArr[0]['id']);
      ...
      console.log(res.data.src);
          ...
          that.setData({
            ...
            notice: notice,
            page: page,
            sousuo: res.data.sousuo,
            index_banner: res.data.src
          })
          that.getToalCartsNum()
